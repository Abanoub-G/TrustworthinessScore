...
def main():
  ...
  if args.model!='-1':
    dnn=load_model(args.model) # I commendted this out
  elif args.vgg16:
    print ('to load VGG16')
    dnn=VGG16()
    print ('done')
  ...
  print("eobj.top_classes = ", eobj.top_classes)
  ...
  print("eobj.measures = ", eobj.measures)

  # print("xs = ",xs)
  # print("xs[0] = ",xs[0])
  print("len(xs) = ", len(xs[0]))

  prediction = eobj.model.predict(xs)
  print("len(prediction[0]) = ", len(prediction[0]))
  print("np.argmax(prediction[0]) = ", np.argmax(prediction[0]))
  print("xs = ",xs)
  # print("-eobj.top_classes = ",-eobj.top_classes)
  ...
  print("max(prediction) = ",max(max(prediction)))
  y=np.argsort(prediction)[0][-eobj.top_classes:]
  print("y = ",y)


  # print("eobj.model.predict(xs) = ",eobj.model.predict(xs))
